...
from . import conf
from .constants import UWSGI_ALIAS_SEPARATOR
...
def get_uwsgi_vassal_opts(settings, port):
    ...
    return ["vassal-set = {}".format(opt.replace(" = ", "=", 1)) for opt in vassal_opts]
...
def get_uwsgi_emperor_opts(settings, port):
    ...
    return get_uwsgi_regular_opts(settings, port) + [
        ...
        "fastrouter-use-code-string = 0:{}:get".format(
            os.path.join(HERE, "fastrouter_lookup.py")
        ),
        "emperor = {}".format(
            pg_str.format(query=VASSALS_SQL_QUERY, **settings["DATABASES"]["default"])
        ),
    ]
...
def start_uwsgi_command(settings, port=None):
    ...
    def _create_alias_domain_mapping_helper_files(settings):
        ...
        connection_string = (
            "dbname='{NAME}' user='{USER}' host='{HOST}' password='{PASSWORD}'".format(
                **db_kwargs
            )
        )
        ...
        for alias, domain in alias_domain_mapping.items():
            filepath = os.path.join(
                alias_domain_mapping_dir, UWSGI_ALIAS_SEPARATOR.join([alias, domain])
            )
