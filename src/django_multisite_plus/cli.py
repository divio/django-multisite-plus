...
from django_multisite_plus.constants import (
...
def get_uwsgi_vassal_opts(settings, port):
    ...
    if (
        ...
        or not settings["STATIC_URL_IS_ON_OTHER_DOMAIN"]
    ):
        # TODO: we're currently starting the static url hosting on all
        # vassals if STATIC_HOST is set. We rely on the fallback for the
        # unknown static domain to end up calling on the main vassal. We
        # should rather do this only on the main vassal or create a custom
        # vassal just for static files.
        serve_static = True
        vassal_opts.extend(
            get_uwsgi_static_serving_opts(
                settings["STATIC_URL"],
...
def get_uwsgi_emperor_opts(settings, port):
    if settings["DATABASES"]["default"].get("PASSWORD"):
        os.environ["DB_PASSWORD"] = settings["DATABASES"]["default"][
            "PASSWORD"
        ]
        pg_str = "pg://host={HOST} port={PORT} user={USER} password=$(DB_PASSWORD) dbname={NAME};{query}"
    else:
        pg_str = (
            "pg://host={HOST} port={PORT} user={USER} dbname={NAME};{query}"
        )

    return get_uwsgi_regular_opts(settings, port) + [
        "http = 0.0.0.0:{}".format(port or settings.get("PORT")),
        # Fastrouter speaks the uwsgi protocol.
        # We need to expose http via http-to.
        "http-to = /tmp/fastrouter.sock",
        "fastrouter = /tmp/fastrouter.sock",
        "fastrouter-use-code-string = 0:{}:get".format(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)),
                "fastrouter_lookup.py",
            )
        ),
        "emperor = {}".format(
            pg_str.format(
                query=VASSALS_SQL_QUERY.format(
                    base_sockets_dir=conf.UWSGI_BASE_SOCKETS_DIR
                ),
...
def assert_uwsgi_plugin_is_installed(plugin):
    # Simply calling "uwsgi --need-plugins=<PLUGIN>" sounds tempting but does
    # not work. For some reason (maybe uwsgi inside virtualenv) the above
    # wouldn't find the plugin even if it's installed.
    plugins = subprocess.check_output(
        ["uwsgi --plugins-list; exit 0"], shell=True, stderr=subprocess.STDOUT
    )
    plugins = plugins.decode("utf-8").split("\n")
    plugins = plugins[: plugins.index("--- end of plugins list ---")]
    plugins = list(
        filter(lambda x: bool(x) and not (x.startswith("***")), plugins)
    )
...
def start_uwsgi_command(settings, port=None):
    def _create_working_dirs():
        for path in (
            conf.UWSGI_BASE_CONFIG_DIR,
            conf.UWSGI_BASE_SOCKETS_DIR,
            conf.UWSGI_ALIAS_DOMAIN_MAPPING_DIR,
        ):
            # Ensure tree is clean before creating the working dirs (in order
            # to avoid orphan vassal sockets).
            try:
                shutil.rmtree(path)
            except OSError:
                pass
    ...
    def _create_uwsgi_config_file(settings, port, uwsgi_config_file_path):
        # We can't use the regular ini writer because it does not allow
        # repeating keys and does not keep order of keys.
        ini = ["[uwsgi]"]
    ...
    def _create_alias_domain_mapping_helper_files(settings):
        # We are spawning 1 uwsgi process per SITE, not per DOMAIN/ALIAS, so we
        # need to map aliases to a proper domain. Here we create a bunch of
        # helper files with filename like "alias|domain" so we can use it as a
        # lookup later.
        ...
        for alias, domain in alias_domain_mapping.items():
            filepath = os.path.join(
                alias_domain_mapping_dir,
                UWSGI_ALIAS_SEPARATOR.join([alias, domain]),
            )
            open(filepath, "w").close()

    uwsgi_config_file_path = os.path.join(
        conf.UWSGI_BASE_CONFIG_DIR, "config.ini"
    )
    ...
    return ["uwsgi", "--ini={}".format(uwsgi_config_file_path)]
