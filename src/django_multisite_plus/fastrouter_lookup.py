...
def get(key_as_bytes):
    def _get_socket(key):
        filepath = os.path.join(
            ...
            "{}{}*".format(key, UWSGI_ALIAS_SEPARATOR),
        )
        sockets = glob.glob(filepath)

        if len(sockets) > 1:
            raise RuntimeError(
                'Multiple sockets found for key="{}" (found: "{}")'.format(
                    key, ", ".join(sockets)
                )
            )
        elif len(sockets) == 1:
    ...
    if conf.UWSGI_LOCAL_TEST_MODE:
        if conf.UWSGI_LOCAL_TEST_KEY:
            ...
            logger.warn(
                'WARNING: UWSGI_LOCAL_TEST_MODE is True! Forcing key="{}"'.format(
                    key_as_text
                )
            )
        else:
            ...
            logger.warn(
                'WARNING: UWSGI_LOCAL_TEST_MODE is True! Using test socket="{}"'.format(
                    fake_socket
                )
            )
    ...
    if conf.UWSGI_DEFAULT_DOMAIN:
        ...
        if not default_socket:
            raise RuntimeError(
                'Default socket not found for domain "{}".'.format(
                    conf.UWSGI_DEFAULT_DOMAIN
                )
            )
